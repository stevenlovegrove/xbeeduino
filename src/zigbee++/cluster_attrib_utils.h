...
inline uint8_t* put_atrib(uint8_t* buffer, const zcl_attribute_base_t* entry, bool include_success, unsigned max_size_bytes)
{
    assert(entry);
    uint8_t* dest = buffer;
...
inline uint8_t* put_attrib_requests(uint8_t* buffer, zcl_attribute_base_t* attrib_table, int16_t requests_size, const uint16_t* requests, unsigned max_size_bytes)
{
    uint8_t* dest = buffer;
...
inline uint8_t* put_attrib_table(uint8_t* buffer, zcl_attribute_base_t* attrib_table, unsigned max_size_bytes)
{
    assert(attrib_table);
    uint8_t* dest = buffer;
...
inline int send_attrib_requests(zcl_command_t& request, zcl_attribute_base_t* attrib_table, int16_t requests_size, const uint16_t* requests)
{
    assert(attrib_table);
...
inline int send_attrib_table_response(zcl_command_t& request, zcl_attribute_base_t* attrib_table)
{
    const unsigned MAX_SIZE=40;
    PACKED_STRUCT{
        zcl_header_response_t header;
...
inline int send_configure_response(zcl_command_t& request, uint8_t status)
{
    const unsigned MAX_SIZE=40;
    PACKED_STRUCT{
        zcl_header_response_t header;
...
inline void report_attribs(xbee_dev_t& xdev, const zcl_attribute_base_t *attr_list, const wpan_endpoint_table_entry_t *source_endpoint) {
